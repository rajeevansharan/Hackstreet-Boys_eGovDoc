...
// Page: Enter Your Mobile Number
// After successful username/password login, user lands here to start phone OTP MFA.
// Attempts to call optional backend endpoint POST /auth/request-phone-otp { phone }
// If backend not implemented yet, falls back to a mock and proceeds to OTP page.
...
export default function EnterPhoneNumber() {
  ...
  const disabled = loading || cleaned.length < 9; // allow 9+ digits
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res || !res.ok) {
        // Fallback mock path (no backend yet)
        console.warn(
          "request-phone-otp endpoint missing; using mock OTP 123456",
        );
        sessionStorage.setItem("egovdoc:mockOtp", "123456");
      }
